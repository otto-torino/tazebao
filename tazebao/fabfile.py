...
def restart_uwsgi():
    ...
    with settings(warn_only=True):
        ...
        sudo('kill -9 `cat /tmp/project-master_tazebao.pid`')
        sudo('rm /tmp/project-master_tazebao.pid /tmp/uwsgi_tazebao.sock')
...
def dump_db_snapshot():
    ...
    sudo('mysqldump --user tazebao --password dbtazebao > %s' % (remote_tmp_file_path))
