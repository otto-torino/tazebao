...
from .serializers import (
    CampaignSerializer,
    DispatchSerializer,
    FailedEmailSerializer,
    MailerMessageSerializer,
    PlanningSerializer,
    SubscriberListSerializer,
    SubscriberSerializer,
    SubscribtionsStatsSerializer,
    SubscriptionFormSerializer,
    TopicSerializer,
    UnsubscriptionStatsSerializer,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.add_message(
                    request, messages.ERROR, "L'email inserita è già presente"
                )
            except Exception as e:
                messages.add_message(
                    request, messages.ERROR, "Si è verificato un errore: %s" % str(e)
                )
    else:
...
def unsubscribe(request):
    ...
    if (
        ...
        and request.GET.get("email", False)
        and request.GET.get("sig", False)
    ):  # noqa
        ...
        signature = unquote_plus(
            encrypt(
                ...
                str(request.GET["id"]) + str(request.GET["email"]),
            )  # noqa
        )
        if sig == signature:
            subscriber.delete()
            return render(
                ...
                {"newsletter": subscriber.client, "email": subscriber.email},
            )
        else:
            raise Http404()
    raise Http404()
...
def campaign_detail_view(request, client_slug, year, month, day, campaign_slug):
    ...
    if request.user == campaign.client.user or campaign.view_online:
        ...
        unsubscribe_url = ""
        if (
            request.GET.get("subscriber", False)
            and request.GET.get("dispatch", False)
            and request.GET.get("sig", False)
        ):  # noqa
            sig = request.GET["sig"]
            signature = unquote(
                encrypt(
                    ...
                    str(request.GET["subscriber"]) + str(request.GET["dispatch"]),
                )  # noqa
            )
            if sig == signature:
                ...
                dispatch = get_object_or_404(Dispatch, id=int(request.GET["dispatch"]))

                unsubscribe_url_template = template.Template(
                    "{% load newsletter_tags %}"
                    + (
                        ""
                        if campaign.topic.unsubscribe_url is None
                        else campaign.topic.unsubscribe_url
                    )  # noqa
                )
                if campaign.topic.unsubscribe_url:
                    ...
                    ctx.update(
                        {"subscription_datetime": subscriber.subscription_datetime}
                    )  # noqa
                    unsubscribe_url = unsubscribe_url_template.render(ctx)

        if (
            campaign.html_text is not None
            and campaign.html_text != ""
            and not request.GET.get("txt", False)
        ):
        ...
        return http.HttpResponse(tpl.render(context), content_type=content_type)
...
def link_tracking(request, dispatch_id, subscriber_id):
    signer = Signer()
    s = signer.sign(
        "%s-%s-%s" % (str(dispatch_id), str(subscriber_id), request.GET.get("url", ""))
    ).rsplit(":", 1)[1]
    if s != request.GET.get("s", ""):
        raise http.Http404()
...
class DynamicPagination(object):
    ...
    def paginator(self):
        ...
        if (
            self.request.query_params.get("page_size", None) is None
            and self.get_for_client().count() < self.pagination_threshold
        ):
...
class SubscriberListViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def get_for_client(self):
        return SubscriberList.objects.filter(client__user__id=self.request.user.id)
...
class ImportSubscribersFromCsv(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(rows) > 1000:
            return Response(
                {
                    "detail": "Limite massimo raggiunto: non puoi importare più di 1000 contatti alla volta"
                },
                status=400,
            )
        try:
...
class PlanningViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def get_for_client(self):
        return Planning.objects.filter(campaign__client__user__id=self.request.user.id)
...
class FailedEmailViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def get_for_client(self):
        return FailedEmail.objects.filter(client__user__id=self.request.user.id)
...
class SubscriberViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            raise ValidationError(
                detail={
                    "email": "Esiste già un utente iscritto con questo indirizzo e-mail"
                }
            )
        ...
        for subscriber_id in subscribers:
            try:
                subscriber = Subscriber.objects.get(
                    id=int(subscriber_id), client=request.user.client
                )
    ...
    def delete_from_bounces(self, request):
        ...
        try:
            Subscriber.objects.filter(
                bounces__id__in=bounces_ids, client=request.user.client
            ).delete()
    ...
    def delete_from_mailermessages(self, request):
        ...
        try:
            emails = [
                m.to_address
                for m in MailerMessage.objects.filter(
                    id__in=ids,
                    app__in=[
                        str(d.pk)
            ...
            Subscriber.objects.filter(
                email__in=emails, client=request.user.client
            ).delete()
    ...
    def delete_many(self, request):
        ...
        try:
            Subscriber.objects.filter(id__in=ids, client=request.user.client).delete()
            return Response({"detail": "subscribers deleted"})
        except Exception as e:
            print(e)
            return HttpResponseBadRequest(str(e))
...
class CampaignViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if date_from is not None:
            qs = qs.filter(
                last_edit_datetime__gte=datetime.strptime(date_from, "%Y-%m-%d")
            )
    ...
    def send(self, request, pk=None):
        ...
        if campaign.client != request.user.client:
            ...
            try:
                ...
                return HttpResponseBadRequest("cannot send campaign: %s" % str(e))
    ...
    def test(self, request, pk=None):
        ...
        if campaign.client != request.user.client:
            ...
            try:
                ...
                tot = sum(
                    [
                        SubscriberList.objects.get(id=l).subscriber_set.count()
                        for l in lists
                    ]
                )
                if tot > settings.TEST_EMAIL_MAX_NUM:
                    return HttpResponseBadRequest(
                        "the max number of allowed e-mails is 50, you tried to send %d"
                        % int(tot)
                    )
                test_campaign.delay(lists, campaign.id)
                return Response({"detail": "task in execution"})
            except Exception as e:
                return HttpResponseBadRequest("cannot test campaign: %s" % str(e))
    ...
    def duplicate(self, request, pk=None):
        ...
        try:
            ...
            return HttpResponseBadRequest("cannot duplicate campaign: %s" % str(e))
...
class DispatchViewSet(DynamicPagination, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if date_from is not None:
            qs = qs.filter(started_at__gte=datetime.strptime(date_from, "%Y-%m-%d"))
        if date_to is not None:
            qs = qs.filter(started_at__lte=datetime.strptime(date_to, "%Y-%m-%d"))
        return qs

    def get_for_client(self):
        return Dispatch.objects.filter(campaign__client__user__id=self.request.user.id)
...
class FailedEmailApiView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(FailedEmailApiView, self).dispatch(request, *args, **kwargs)
...
class TopicViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def get_for_client(self):
        return Topic.objects.filter(client__user__id=self.request.user.id)
    ...
    def get(self, request):
        if not request.user.is_authenticated:
            ...
            last_month_subscribers = Subscriber.objects.filter(
                client__user=request.user, subscription_datetime__gte=last_month
            ).count()
            last_month_unsubscriptions = Unsubscription.objects.filter(
                client__user=request.user, datetime__gte=last_month
            ).count()
            last_dispatch = Dispatch.objects.filter(
                campaign__client__user=request.user, test=False
            ).last()
            next_planning = Planning.objects.filter(
                campaign__client__user=request.user, schedule__gte=datetime.now()
            ).first()
            tot_bounces = FailedEmail.objects.filter(client__user=request.user).count()
            tot_dispatching = MailerMessage.objects.filter(
                app__in=[
                    str(d.pk)
                    for d in Dispatch.objects.filter(
                        campaign__client__user=self.request.user, sent=False
                    )
                ]
            ).count()
            ...
            response = {
                ...
                "bounces": tot_bounces,
                "dispatching": tot_dispatching,
                "lastMonthUnsubscriptions": last_month_unsubscriptions,
                "lastMonthSubscribers": last_month_subscribers,
                "lastDispatch": DispatchSerializer(last_dispatch).data
                if last_dispatch
                else None,
                "nextPlanning": PlanningSerializer(next_planning).data
                if next_planning
                else None,
            }
        return Response(response)
...
class MailerMessageViewSet(
    ...
    def get_for_client(self):
        return MailerMessage.objects.filter(
            app__in=[
                str(d.pk)
                for d in Dispatch.objects.filter(
...
class SubscriptionsStatsApiView(APIView):
    def get(self, request):
        ...
        sub_data = (
            Subscriber.objects.filter(client__user=request.user)
            .values("subscription_datetime__date")
            .exclude(subscription_datetime__date__isnull=True)
            .order_by("subscription_datetime__date")
            .annotate(cnt=Count("subscription_datetime__date"))
        )
        unsub_data = (
            Unsubscription.objects.filter(client__user=request.user)
            .values("datetime__date")
            .exclude(datetime__date__isnull=True)
            .order_by("datetime__date")
            .annotate(cnt=Count("datetime__date"))
        )

        return JsonResponse(
            {
                "subscriptions_stats": SubscribtionsStatsSerializer(
                    sub_data, many=True
                ).data,
                "unsubscriptions_stats": UnsubscriptionStatsSerializer(
                    unsub_data, many=True
                ).data,
            }
        )
...
class DispatchStatsApiView(APIView):
    def get(self, request):
        ...
        dispatches = Dispatch.objects.filter(
            campaign__client__user=request.user, campaign__id__in=ids, test=False
        )
...
class SubscriptionFormViewSet(DynamicPagination, viewsets.ModelViewSet):
    ...
    def get_for_client(self):
        return SubscriptionForm.objects.filter(client__user__id=self.request.user.id)
...
class SubjectSuggestionApiView(APIView):
    def post(self, request):
        ...
        today_suggestions = SuggestionRequest.objects.filter(
            client__user=request.user,
            datetime__date=datetime.now().strftime("%Y-%m-%d"),
        ).count()
        if today_suggestions >= request.user.client.suggestions_per_day:
            return JsonResponse(
                {"description": "too many suggestions today"}, status=400
            )
        ...
        model = "gpt-4o-mini"
        role = "Sei un assistente di marketing esperto e professionale"
        question = (
            "Suggeriscimi 5 titoli di massimo 50 caratteri di articoli newsletter accattivanti che parlano di %s rivolti ad un pubblico di eta media di %d anni. Dammi solo i titoli, uno per riga e senza usare virgolette."
            % (topic, mean_age)
        )
        messages = []
        ...
        suggestion_request.answer = answer
        suggestion_request.save()
...
class SystemMessageViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = SystemMessage.objects.filter(
            Q(clients__user__in=[self.request.user]) | Q(clients=None)
        ).exclude(
            id__in=[
                SystemMessageRead.objects.filter(client__user=self.request.user).values(
                    "system_message_id"
                )
            ]
        )
